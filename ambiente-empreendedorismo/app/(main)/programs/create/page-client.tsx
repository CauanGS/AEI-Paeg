...
export default function CreateProgramPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/programs', { method: 'POST', body: data });
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.error || 'Falha ao criar programa');
      }
  ...
  if (!allowed) {
    return <div className="py-20 text-center text-lg text-gray-600 bg-gray-50 min-h-screen"></div>;
  }

  return (
    <div className="bg-gray-50 py-12">
      <div className="container mx-auto max-w-3xl p-6 bg-white shadow-lg rounded-lg text-gray-900">
        <h1 className="text-4xl font-bold text-[#2E2B82] text-center mb-8">Criar Programa</h1>

        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label htmlFor="title" className="block text-sm font-medium text-gray-700 mb-1">Título</label>
            <input id="title" name="title" value={formData.title} onChange={handleInputChange} placeholder="Título do programa" className="w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-[#2E2B82] focus:border-[#2E2B82] text-gray-900 bg-white" required />
          </div>

          <div>
            <label htmlFor="description" className="block text-sm font-medium text-gray-700 mb-1">Descrição Curta</label>
            <textarea id="description" name="description" rows={3} value={formData.description} onChange={handleInputChange} placeholder="Descrição breve do programa (para exibição em cards)" className="w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-[#2E2B82] focus:border-[#2E2B82] text-gray-900 bg-white" required />
          </div>
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">Selecione uma imagem para o programa (opcional).</p>
          </div>
